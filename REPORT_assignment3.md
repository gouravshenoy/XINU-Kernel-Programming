...
Once the producer code is done using the critical section, it calls signal(produced) so that the first process(consumer code in this case) waiting on the semaphore is moved to the ready queue. Similarly, when the consumer code is done using the critical section for each iteration, it calls signal(consumed), so that the first process(producer process in this case) waiting on the semaphore is moved to the ready queue
